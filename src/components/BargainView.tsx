...
export function BargainView({ state, onTakeBargain, onRestart }: BargainViewProps) {
  return (
    <div className="game-screen">
      <Hud state={state} onRestart={onRestart} />
      <section className="bargain-room" aria-label="Cursed bargain room">
        <div className="altar-sprite" aria-hidden="true">
          <span />
        </div>
        <div className="bargain-copy">
          <span>Cursed Altar</span>
        ...
        <div className="bargain-list">
          {bargainOptions.map((option) => (
            <button
              key={option.id}
              type="button"
              className="bargain-card"
              onClick={() => onTakeBargain(option.id)}
            >
